...
def on_message(client, userdata, msg):
   ...
   gdown.download(url, "tabsheet.jpg", quiet=False)
   print('Image downloaded from mobile...')


client = connect_mqtt()
client.subscribe("tabletUsageAnalyserOS")
...
def submit(filename):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            print("COUNTS => ", s)
            client.publish("tabletUsageAnalyserIS", "Counts = " + s )
            print('Result sent back to mobile..')
            print(f'{s}Done. ({(1E3 * (t2 - t1)):.1f}ms) Inference, ({(1E3 * (t3 - t2)):.1f}ms) NMS')
            os.remove("tabsheet.jpg")
